...
class VectorizedSampler(BaseSampler):
    ...
    def obtain_samples(self, itr, reset_args=None, return_dict=False, log_prefix=''):
        ...
        while n_samples < self.algo.batch_size:
            ...
            t = time.time()
            next_obses, rewards, dones, env_infos = self.vec_env.step(pro_actions, adv_actions, reset_args)
            ...
            for idx, observation, pro_action, adv_action,, reward, env_info, pro_agent_info, adv_agent_info, done in zip(itertools.count(), obses, pro_actions,
                ...
                if done:
                    paths[idx].append(dict(
                        ...
                        pro_actions=self.env_spec.pro_action_space.flatten_n(running_paths[idx]["pro_actions"]),
                        adv_actions=self.env_spec.adv_action_space.flatten_n(running_paths[idx]["adv_actions"]),
